#' Server logic for the pilot app
#'
...
app_server <- function(input, output, session) {
  ...
  userInfo <-
    rapbase::howWeDealWithPersonalData(session, callerPkg = "pilot")
  ...
  output$veiledning <- shiny::renderUI({
    rapbase::renderRmd(
      system.file("veiledning.Rmd", package = "pilot"),
  ...
  output$samlerapport <- shiny::renderUI({
    rapbase::renderRmd(
      system.file("samlerapport.Rmd", package = "pilot"),
      outputType = "html_fragment",
  ...
  output$downloadSamlerapport <- shiny::downloadHandler(
    filename = function() {
      basename(tempfile(pattern = "pilotSamlerapport",
                        fileext = paste0(".", input$formatS)))
    },
    content = function(file) {
      srcFile <-
        normalizePath(system.file("samlerapport.Rmd", package = "pilot"))
  ...
  rapbase::autoReportServer(
    id = "testSubscription", registryName = "pilot",
